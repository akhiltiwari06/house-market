...
const EditListing = () => {
  ...
  const [geoLoactionEnable, setGeoLocationEnable] = useState(true); // eslint-disable-line
  ...
  const onSubmit = async (e) => {
    ...
    if (geoLoactionEnable) {
      const response = await fetch(
        `https://maps.googleapis.com/maps/api/geocode/json?address=${address}&key=AIzaSyALu3IyCp5UdUEcxSqzq-7tQIb9ogYoh9Q`
      );
